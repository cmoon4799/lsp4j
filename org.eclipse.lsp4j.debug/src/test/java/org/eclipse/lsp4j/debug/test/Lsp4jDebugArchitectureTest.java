...
/**
 * Test to make sure we don't depend to xbase.lib, xtend.lib and guava at runtime
 * 
...
public class Lsp4jDebugArchitectureTest {
	...
	private void assertNoDependencyToPackage(String badPackage) {
		ArchRule rule = noClasses().that(resideInAPackage("org.eclipse.lsp4j.debug..").and(
				not(resideInAPackage("org.eclipse.lsp4j.generator.."))))
				.and(
						not(resideInAPackage("org.eclipse.lsp4j.debug.test..")))
				.should().dependOnClassesThat().resideInAPackage(badPackage);
